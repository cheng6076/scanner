...
class LSTMParser(object):
    ...
    def __init__(self, word_vocab, nt_vocab, ter_vocab, act_vocab, word_dim, nt_dim, ter_dim, lstm_dim, nlayers, order='pre_order', embedding_file=None, attention='bilinear'):
    ...
    def parse(self, words, oracle_actions, oracle_tokens):
        ...
        if self.order == 'pre_order':
            output_actions, output_tokens = self.pre_order_parse(words, oracle_actions, oracle_tokens, buffer, stack_top, action_top)
        elif self.order == 'post_order':
            output_actions, output_tokens = self.post_order_parse(words, oracle_actions, oracle_tokens, buffer, stack_top, action_top)
        elif self.order == 'span':
            output_actions, output_tokens = self.span_parse(words, oracle_actions, oracle_tokens, buffer, stack_top, action_top)
        else:
            raise NotImplementedError("Generation order not supported")
    ...
    def pre_order_train(self, words, oracle_actions, oracle_tokens, options, buffer, stack_top, action_top):
        ...
        while not (len(stack) == 1 and reduced != 0):
            ...
            if len(stack) > 0 and valid_actions[0] != _ACT:
                ...
                buffer_embedding = dy.esum([vector*attterion_weight for vector, attterion_weight in zip(buffer, word_weights)])
