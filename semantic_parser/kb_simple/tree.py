    ...
    def top_down(self, identifier, general_nts):
        ...
        def recurse(identifier):
            ...
            if len(node.children) == 0:
                if node.content.startswith('rel.'):
                    data.append(node.content)
                    action.append('TER(relation)')
                elif node.content.startswith('ent.'):
                    data.append(node.content)
                    action.append('TER(entity)')
            else:
                if node.content in general_nts:
                    action.append("{}({})".format('NT', node.content))
                else:
                    action.append('NT(relation)')
                data.append(node.content)
...
def test():
    ...
    s = 'and(rel.born(ent.seattle) rel.job(ent.uni_teacher))'
    s = 'answer(argmax(rel.salary))' 
    x = Tree()
